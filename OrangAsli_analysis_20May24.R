...
#rename villages
register$name[register$name == 'Hulu Tamu (Batang Kali)'] <- 'Hulu Tamu'
register$name[register$name == 'Tual (B)'] <- 'Tual'
...
register2=select(register, c("village_id", "name", "lat", "long"))
...
ggsave(plot, file="Fig2_Food.pdf", height=6, width =12, useDingbats=FALSE)
...
plot = ggplot(WaistCirc, aes(y=disp, x=reorder(name,-lat), fill = name)) + geom_pointrange(aes(ymin = lowsd, ymax = highsd)) + 
...
plot
ggsave(plot, file="Fig4_WaistCirc.pdf", height=6, width =8, useDingbats=FALSE)
...
plot = ggplot(WaistCirc, aes(y=disp, x=reorder(name,-lat), fill = name)) + geom_pointrange(aes(ymin = lowsd, ymax = highsd)) + 
  xlab("Location") +ylab('Waist circumference') +
  theme_classic() + 
  facet_wrap(~sex,labeller = labeller(sex = c("0" = "Female","1" = "Male"))) + scale_color_manual(values=c("#31ca31","#228b22","#f36500", "#31ca31","#b989b9","#31ca31","#b989b9","#8d548d","#228b22","#228b22"))+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+ theme(text = element_text(size=20), legend.position = "none", plot.title = element_text(hjust = 0.5)) +
